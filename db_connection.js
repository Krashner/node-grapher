...
if (process.env.MYSQL_INSTANCE_CONNECTION_NAME && process.env.NODE_ENV === 'production') { // database connection for production
  config = {
...
else{ // database connection for development
  config = {
...
connection.connect(function(err) { // actually attempt to make the connection
  if (err) {
    console.error('Error connecting: ' + err.stack);
    return;
  }
  console.log(`Connected to ${process.env.MYSQL_DATABASE_NAME} database with thread ID: ` + connection.threadId);
});
